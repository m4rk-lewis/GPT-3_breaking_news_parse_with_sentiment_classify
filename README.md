...
::: {.cell .code execution_count="2" id="Pc3S-XJVBaIb"}
``` python
...
::: {.cell .code execution_count="3" id="W5XIA-eyBfJU"}
``` python
...
::: {.cell .code execution_count="23" id="r-NlATswBhpj"}
``` python
import openai
openai.api_key = "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"
...
::: {.cell .code execution_count="5" id="4JbNpYxYBoo0"}
``` python
...
::: {.cell .code execution_count="24" id="BsdXg5TLBtdT"}
``` python
...
::: {.cell .code execution_count="25" colab="{\"base_uri\":\"https://localhost:8080/\",\"height\":932}" id="BtUcnCZRBxoU" outputId="b7dde84f-f692-444f-bedf-aed04072e857"}
``` python
'''
...
::: {.output .error ename="KeyboardInterrupt" evalue="ignored"}
    ---------------------------------------------------------------------------
    KeyboardInterrupt                         Traceback (most recent call last)
    <ipython-input-25-95e451f0c590> in <cell line: 5>()
          6     url = "https://www.forexlive.com/feed"
          7     conn, cursor = create_db()
    ----> 8     monitor_feed(url, conn, cursor)

    <ipython-input-24-63aa925103a5> in monitor_feed(url, conn, cursor, interval)
         15                 insert_news(cursor, title, summary, sentiment)
         16                 print(f"Inserted news: {title} : {summary} : {sentiment}")
